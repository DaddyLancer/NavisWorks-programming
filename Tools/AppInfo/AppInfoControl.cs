...
namespace AppInfo.Control
{
   public partial class AppInfoControl : UserControl
   {
      ImageList formIcons = new ImageList();
      ...
      public AppInfoControl()
      {
         ...
         appView.ImageList = formIcons;
         propertiesView.SmallImageList = formIcons;
      ...
      private void AppInfoControl_Load(object sender, EventArgs e)
      {
         try
         {
            AddRootNode(typeof(Autodesk.Navisworks.Api.Application), "Application", null);
         }
         catch (Exception ex)
         {
         ...
         if (e.Button == MouseButtons.Right)
         {
            if (eventsView.SelectedItems.Count > 0)
            {
               if (eventsView.SelectedItems[0].Tag is EventInfo)
               {
                  ...
                  if (nodeInfo != null)
                  {
                     attachToEventToolStripMenuItem.Enabled = !EventHandlers.ContainsKey(nodeInfo.Type, nodeInfo.Value, eventsView.SelectedItems[0].Text);
      ...
      void EventHandlers_EventRaised(object sender, EventDetailsArgs e)
      {
         ...
         PropertyInfo[] propertyInfo = e.EventArgs.GetType().GetProperties(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
         // sort by name
      ...
      /// 
      /// </summary>
      private class NodeInfo : IDisposable
      {
      ...
      private bool CheckNodeIsValid(TreeNode currNode)
      {
         ...
         Type parentType = GetTypeInformation(parentNode, out parentObj, out isParentDisposed, out isParentStatic, out parentIndex, out parentInfo);
         ...
         MemberInfo currInfo;
         Type nodeType;
         Type currType = GetTypeInformation(currNode, out currObj, out isCurrDisposed, out isCurrStatic, out currIndex, out currInfo, out nodeType);
      ...
      #region Add / Update /Remove Nodes
      ...
      private static void AddNode(TreeNode parentNode, string nodeText, object o, Type type, TypeIcon imageIndex, bool isStatic, MemberInfo info)
      {
         ...
         try
         {
              ...
              info.CanWrite && info.GetSetMethod().IsStatic)
            {
               AddNode(parentNode, info.Name, o, info.PropertyType, TypeIcon.staticproperty,
                  (info.CanRead && info.GetGetMethod().IsStatic || info.CanWrite && info.GetSetMethod().IsStatic), info);
            }
            else
            {
      ...
      private static void AddNode(TreeNode parentNode, string nodeText, object o, Type type, TypeIcon imageIndex, bool isStatic, MemberInfo info, int index)
      {
      ...
      {
         foreach (TreeNode node in appView.Nodes)
         {
            object currObj = null;
            bool isDisposed = false;
            bool isStatic = false;
            GetTypeInformation(node, out currObj, out isDisposed, out isStatic);
            if (currObj == rootObject)
            {
               appView.Nodes.Remove(node);
               break;
            }
         }
      }
      ...
      private void ExpandNode(TreeNode currNode)
      {
         try
         {
            if (currNode != null)
            {
               ...
               {
                  //public fields
                  FieldInfo[] fieldInfo = currType.GetFields(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
                  // sort by name
                  ...
                  //public properties
                  PropertyInfo[] propertyInfo = currType.GetProperties(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
                  // sort by name
                  Array.Sort(propertyInfo, MemberCompare);
                  ...
                  foreach (GetMemberBinder binder in GetDynamicMemberBinders(currObj))
                  {
                         ...
                         sub_obj.GetType().IsRelatedTo(ref sought_type))
                     {
                        ...
                        {
                           AddNode(currNode, binder.Name + "[" + i.ToString() + "]", sub_obj, sought_type, TypeIcon.pubproperty, false, null);
                        }
                     }
                  }
               }
      ...
      private void DisplayNodeProperties(TreeNode currNode)
      {
         if (currNode != null)
         {
            ...
            else if(currType != null)
            {
               ...
               AddToListView(propertiesView, PropertyGroup.Value, isStatic, (currObj != null ? currObj.ToString() : "null"), null);

               DisplayFields(currNode, currType, currObj);

               DisplayProperties(currNode, currType, currObj);

               DisplayMethods(currNode, currType);

               DisplayDynamicMembers(currNode, currType, currObj);

               DisplayEvents(currNode, currType, currObj);
            }
         }
      }

      private void DisplayFields(TreeNode currNode, Type currType, object currObj)
      {
         //Fields
         FieldInfo[] fieldInfo = currType.GetFields(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
         // sort by name
         ...
         foreach (FieldInfo info in fieldInfo)
         {
            try
            {
               AddToListView(propertiesView, PropertyGroup.Property, info.IsStatic, info.Name, info.GetValue(currObj).ToString(), null);
            }
            catch (Exception)
            { }
         }
      ...
      private void DisplayProperties(TreeNode currNode, Type currType, object currObj)
      {
         //Properties
         PropertyInfo[] propertyInfo = currType.GetProperties(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
         // sort by name
            ...
            try
            {
               if (info.CanRead)
               {
                  ...
                  {  // is this browsable?
                     ...
                     System.ComponentModel.EditorBrowsableAttribute attrib = attributes[0] as System.ComponentModel.EditorBrowsableAttribute;
                     System.Diagnostics.Debug.Assert(attrib != null);
                     browsable = (attrib.State == System.ComponentModel.EditorBrowsableState.Always);
                  }
                  if(browsable)
                  {  bool is_static = (info.CanRead && info.GetGetMethod() != null && info.GetGetMethod().IsStatic);
                     ...
                     AddToListView(propertiesView, PropertyGroup.Property, is_static, info.Name, null, current_value);
                  }
               }
            }
            catch (Exception)
      ...
      private void DisplayDynamicMembers(TreeNode currNode, Type currType, object currObj)
      {
         foreach (GetMemberBinder binder in GetDynamicMemberBinders(currObj))
         {
            CallSite<Func<CallSite, object, object>> site = CallSite<Func<CallSite, object, object>>.Create(binder);
            AddToListView(propertiesView, PropertyGroup.Dynamic, false, binder.Name, null, ConvertDynamicObject(site.Target(site, currObj)));
         }
      }
      ...
      private IEnumerable<GetMemberBinder> GetDynamicMemberBinders(object obj)
      {
         if (obj != null && obj.GetType().IsImplementationOf(typeof(IDynamicMetaObjectProvider)))
         {
            ...
            {
               GetMemberBinder binder = (GetMemberBinder)Microsoft.CSharp.RuntimeBinder.Binder.GetMember(CSharpBinderFlags.None, member, this.GetType(), new[] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) });
               yield return binder;
            }
         }
      }
      ...
      private string ConvertDynamicObject(object dynamic_obj)
      {
         ...
         if (dynamic_obj.GetType().IsRelatedTo(ref sought_type) || dynamic_obj.GetType().IsRelatedTo(ref sought_type2))
         {
            StringBuilder str_builder = new StringBuilder();
            IEnumerable list = (IEnumerable)dynamic_obj;
            foreach(object list_value in list)
            {
               str_builder.Append(list_value.ToString());
               str_builder.Append(", ");
            }
      ...
      private void DisplayMethods(TreeNode currNode, Type currType)
      {
         //Methods
         System.Reflection.MethodInfo[] methodInfo = currType.GetMethods(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
         // sort by name
      ...
      private void DisplayEvents(TreeNode currNode, Type currType, object currObj)
      {
         //Events
         System.Reflection.EventInfo[] eventInfo = currType.GetEvents(BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static);
         // sort by name
         ...
         foreach (System.Reflection.EventInfo info in eventInfo)
         {
            try
            {
               ...
               ListViewItem lvi = AddToListView(eventsView, PropertyGroup.Event, false, info.Name, info, statusText);
      ...
      private ListViewItem AddToListView(ListView lv, PropertyGroup group, bool isStatic, string column1, object tag, params string[] columns)
      {
      ...
      private static Regex _regex = new Regex("(.*?System\\.Collections\\.(Generic.+\\.*|ObjectModel\\..*Collection\\.*)?)(`[0-9]+|\\^)(\\[)(.+)(\\])");
      ...
      /// <param name="index"></param>
      /// <returns></returns>
      private Type GetTypeInformation(TreeNode currNode, out object currObj, out bool isDisposed, out bool isStatic, out int index)
      {
         MemberInfo info;
      ...
      /// <returns></returns>
      private Type GetTypeInformation(TreeNode currNode, out object currObj, out bool isDisposed, out bool isStatic, out int index, out MemberInfo info)
      {
         Type nodeType;
         return GetTypeInformation(currNode, out currObj, out isDisposed, out isStatic, out index, out info, out nodeType);
      }
      ...
      /// <returns></returns>
      private Type GetTypeInformation(TreeNode currNode, out object currObj, out bool isDisposed, out bool isStatic, out int index, out MemberInfo info, out Type nodeType)
      {
         Type currType = null;
         isDisposed = true;
         isStatic = false;
      ...
      #region Output Window Context Menu
   ...
   internal static class TypeExtensions
   {
      ...
      public static bool IsRelatedTo(this Type tested_type, ref Type related_type)
      {
         ...
         {
            if (tested_type == related_type || (tested_type.IsGenericType && tested_type.GetGenericTypeDefinition() == related_type))
            {
